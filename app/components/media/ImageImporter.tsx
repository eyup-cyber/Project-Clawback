...
export default function ImageImporter({ onImageSelect, currentImage }: ImageImporterProps) {
  ...
  const handleUploadAndSelect = async (imageUrl: string) => {
    ...
    try {
      ...
      const uploadedImage = data.data.images?.find(
        (img: any) => img.originalUrl === imageUrl || img.url === imageUrl
      );
    ...
    <div className="space-y-4">
      ...
      {images.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm opacity-70">Found {images.length} image(s). Click to select:</p>
      ...
      <div className="text-xs opacity-60 space-y-1">
        <p>
          💡 <strong>Tip:</strong> Paste a Patreon post URL or X (Twitter) post URL to import
          images.
        </p>
